...
class Home extends Core_Controller {
	public function __construct(){
		...
		$config = Array(
			...
			'smtp_port' => 465,
			'smtp_user' => 'tkelektronikbsi@gmail.com',
			'smtp_pass' => 'clmddwenjqowtvpb',
			'mailtype'  => 'html', 
			'charset'   => 'iso-8859-1'
		);
		...
		if($judul=="login"){					
			...
			if($data){
				...
				if($data2){
					...
					$this->email->from('radenahmad32@gmail.com');
					$this->email->to($email);
					$this->email->subject('Informasi Akun');
					$this->email->set_mailtype("html");
					$this->email->message($message);
					$this->email->send();
					echo "OK|Terima kasih Akun Sudah Terdaftar!\nUserid dan Password anda telah kami kirim ke : ".$email;						
				}				
			}			
			...
			if($email==''){}else{
				...
				$messagex.="Detail Item :<br>".$message."<br/><br/>";
				$messagex.="</body></html>";
				$this->email->from('tkelektronikbsi@gmail.com');
				$this->email->to($email);
				$this->email->subject($title);
				$this->email->set_mailtype("html");
				$this->email->message($messagex);
